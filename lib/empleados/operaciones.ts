...
export async function getSatisfaccionOperaciones(): Promise<{ promedio100: number | null; tareas: number; validadoEn: string | null }> {
  if (!URL || !KEY) return { promedio100: null, tareas: 0, validadoEn: null }
  try {
    const r = await fetch(`${URL}/rest/v1/satisfaccion_general?select=satisfaccion_promedio,tareas_calificadas`, {
      headers: { apikey: KEY, Authorization: `Bearer ${KEY}` },
      cache: "no-store",
    })
    if (!r.ok) return { promedio100: null, tareas: 0, validadoEn: null }
    const rows = await r.json()
    const row = Array.isArray(rows) ? rows[0] : rows
    const prom = Number(row?.satisfaccion_promedio) || 0
    const tareas = Number(row?.tareas_calificadas) || 0
    // La vista da 1..5; lo llevamos a 0..100 para el termómetro. La lectura es en vivo,
    // así que la fecha de validación es el momento de esta consulta (hora de Bogotá).
    const validadoEn = new Date().toLocaleString("es-CO", { timeZone: "America/Bogota", dateStyle: "medium", timeStyle: "short" })
    return { promedio100: prom > 0 ? Math.round(prom * 20) : null, tareas, validadoEn: prom > 0 ? validadoEn : null }
  } catch {
    return { promedio100: null, tareas: 0, validadoEn: null }
  }
}
