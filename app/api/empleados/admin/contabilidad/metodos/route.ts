...
import { listMetodosPago, crearMetodoPago } from "@/lib/empleados/contabilidad-queries"
...
export async function GET() {
  ...
  try {
    return NextResponse.json({ metodos: await listMetodosPago() })
  } catch (e) {
    const f = faltaTabla(e)
    return NextResponse.json({ error: f.msg, metodos: [] }, { status: f.status })
  }
}

const schema = z.object({ nombre: z.string().trim().min(1).max(120) })
...
export async function POST(req: Request) {
  ...
  try {
    return NextResponse.json({ metodos: await crearMetodoPago(b.nombre) })
  } catch (e) {
    const f = faltaTabla(e)
