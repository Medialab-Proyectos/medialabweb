...
import { getFactura, subirSoporteFactura, getArchivoFactura } from "@/lib/empleados/freelance-queries"
...
/** POST multipart (campo "archivo"): sube el soporte de prestaciones sociales. */
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const factura = await subirSoporteFactura(a.factura!, bytes, mime)
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const a = await autorizar(id)
  if (a.error) return a.error
  if (!a.factura!.soporte_path) return NextResponse.json({ error: "Sin soporte." }, { status: 404 })

  const { bytes, mime } = await getArchivoFactura(a.factura!.soporte_path)
  return new Response(Buffer.from(bytes), {
    headers: {
      "Content-Type": mime,
      "Content-Disposition": `inline; filename="soporte-${a.factura!.id}"`,
