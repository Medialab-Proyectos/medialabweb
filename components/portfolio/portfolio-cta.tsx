        ...
        <div className={`relative overflow-hidden rounded-3xl p-10 md:p-16 flex flex-col items-center text-center gap-8 text-white transition-all duration-700 ${visible ? "opacity-100 scale-100" : "opacity-0 scale-95"}`} style={{ background: "linear-gradient(135deg, oklch(0.10 0 0) 0%, oklch(0.06 0 0) 100%)" }}>
          ...
          <div className="relative z-10 flex flex-col items-center gap-6 max-w-2xl">
            ...
            <h2 id="portfolio-cta-heading" className="font-display font-bold text-3xl md:text-5xl leading-tight text-white text-balance">
              Tu producto podría ser el próximo caso de éxito.
            </h2>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-white/60">
              <span className="flex items-center gap-1.5"><CheckCircle2 size={14} className="text-green-400" />Consulta gratuita</span>
              <span className="flex items-center gap-1.5"><Zap size={14} className="text-[var(--orange)]" />Respuesta en 24h</span>
              <span className="flex items-center gap-1.5"><Shield size={14} className="text-[var(--cyan)]" />NDA disponible</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 pt-2">
              <BookingModal>
                <button type="button" className="group flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-bold text-[15px] bg-[var(--magenta)] text-white transition-all hover:brightness-110 active:scale-95 shadow-lg shadow-[var(--magenta)]/25 animate-pulse-glow">
                  Quiero transformar mi producto <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                </button>
              </BookingModal>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-white/10 mt-2">
              ...
              <p className="text-xs text-white/50"><span className="text-white font-medium">40+ equipos</span> ya transformaron su producto con nosotros.</p>
            </div>
          </div>
        </div>
      </div>
