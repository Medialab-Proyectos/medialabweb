 ...
 * última actualización y un contador a la próxima nota (agente diario 6:00 a.m.
...
export function NextMatchBar({ updatedAt }: { updatedAt?: string }) {
  ...
  useEffect(() => {
    const tick = () => {
      const now = new Date()
      // 6:00 a.m. Colombia = 11:00 UTC. Próxima ocurrencia.
  ...
  return (
    <div className="sticky top-16 z-40 border-b border-border/60 bg-[var(--surface-dark)]/95 backdrop-blur-sm md:top-20">
      <div className="mx-auto flex max-w-5xl items-center gap-x-5 gap-y-1 overflow-x-auto px-6 py-2 text-xs">
        ...
        <span className="inline-flex shrink-0 items-center gap-1.5 text-muted-foreground">
          <Clock size={12} />
          {t("Próxima nota del radar en", "Next radar note in")}{" "}
          <strong className="tabular-nums text-foreground">{countdown}</strong>
        </span>
        {updated && (
          <span className="hidden shrink-0 items-center gap-1.5 text-muted-foreground sm:inline-flex">
            <RefreshCw size={12} />
